...
class AgentSelfMonitor:
    ...
    async def _analyze_agent_state(self) -> Dict[str, Any]:
        ...
        time_since_progress = (current_time - self.agent_state.last_progress_time).total_seconds()
        
        stuck_reasons = []
        
        # Check for repeated actions
        if len(self.agent_state.repeated_actions) >= self.max_repeated_actions:
            stuck_reasons.append(f"Repeated same action {len(self.agent_state.repeated_actions)} times")
        ...
        if self.agent_state.iteration_count > self.max_iterations_without_progress:
            time_since_start = (current_time - self.agent_state.start_time).total_seconds()
            if time_since_start > 60:  # Only flag if more than 1 minute has passed
                stuck_reasons.append(f"High iteration count ({self.agent_state.iteration_count}) without resolution")
...
Respond with just the intervention type and a brief explanation."""
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if time_since_progress < 60 and self.agent_state.error_count == 0:
            ...
            status = "ok"
        elif len(self.agent_state.repeated_actions) < 3 and self.agent_state.error_count < 3:
            status = "warning"
        else:
            status = "critical"
